...
let secret = 'secret'; // 设置访问配置界面的密钥

// NAT64 服务商配置
//来源nat64.xyz
//https://github.com/level66network/nat64.xyz
const NAT64_PROVIDERS = [
    {
        name: "Kasper Dupont (Germany)",
        country: "Germany / Nürnberg",
        dns64: "2a00:1098:2b::1",
        prefix: "2a00:1098:2b::/96",
        dot: "dot.nat64.dk"
    },
    {
        name: "Kasper Dupont (UK London)",
        country: "United Kingdom / London",
        dns64: "2a00:1098:2c::1",
        prefix: "2a00:1098:2c:1::/96",
        dot: "dot.nat64.dk"
    },
    {
        name: "Kasper Dupont (Finland)",
        country: "Finland / Helsinki",
        dns64: "2a01:4f8:c2c:123f::1",
        prefix: "2a01:4f8:c2c:123f:64::/96",
        dot: "dot.nat64.dk"
    },
    {
        name: "level66.services",
    ...
    },
    {
        ...
        prefix: "2602:fc59:11:64::/96",
        dot: ""
    },
    {
        name: "Default (cmliussss)",
        country: "Default",
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            userID = clientId; // 保持向后兼容
            secret = authKey; // 保持向后兼容
            ...
            // 检查是否使用NAT64代理
            const hasNAT64Param = url.searchParams.has('nat64');
            
            // 如果没有设置proxyIP且不是NAT64模式，使用默认代理IP池
            if ((!proxyIP || proxyIP === '') && !hasNAT64Param) {
            ...
            // 处理代理IP设置 - 与html.js保持一致
            let proxyPort = '443';
            const { proxyip: envProxyIP } = env;
            
            if (envProxyIP) {
                if (envProxyIP.includes(']:')) {
                    let lastColonIndex = envProxyIP.lastIndexOf(':');
                    proxyPort = envProxyIP.slice(lastColonIndex + 1);
                    proxyIP = envProxyIP.slice(0, lastColonIndex);
                } else if (!envProxyIP.includes(']:') && !envProxyIP.includes(']')) {
                    [proxyIP, proxyPort = '443'] = envProxyIP.split(':');
                } else {
                    proxyPort = '443';
                    proxyIP = envProxyIP;
                }
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                if (路径 == '/') {
                ...
                } else if (url.pathname == `/${secret}/edit`) {
                    // 通过 secret/edit 访问配置界面
                    return await KV(request, env);
                } else if (url.pathname == `/${secret}/bestip`) {
                    // 通过 secret/bestip 访问最佳IP
                    return await bestIP(request, env);
                } else if (url.pathname == `/${secret}/nat64`) {
                    // 通过 secret/nat64 访问NAT64配置
                    return await NAT64Config(request, env);
                } else if (url.pathname == `/${secret}/select`) {
                    // 通过 secret/select 访问配置工具页面
                    ...
                    return await bestIP(request, env);
                } else if (url.pathname == `/${动态UUID}/nat64` || 路径 == `/${userID}/nat64`) {
                    return await NAT64Config(request, env);
                } else if (url.pathname == `/${动态UUID}/select` || 路径 == `/${userID}/select`) {
                    // 通过 userID/select 访问配置工具页面
                    return await handleSelectPage(request);
                } else if (url.pathname.match(/^\/IP\/[^\/]+$/)) {
                    // 多API整合模式
                    return await handleMultiApiIntegration(request, url);
                } else if (url.pathname.match(/^\/[a-zA-Z]{2}\/[^\/]+$/)) {
                    // 国家API模式
                    return await handleCountryAPI(request, url);
                } else if (url.pathname.match(/^\/bestip\/[^\/]+$/)) {
                    // 全球最佳IP API
                    return await handleBestIPAPI(request, url);
                } else if (url.pathname.match(/^\/([^\/]+)\/[^\/]+$/) && !url.pathname.includes('/select') && !url.pathname.includes('/edit') && !url.pathname.includes('/bestip') && !url.pathname.includes('/nat64')) {
                    // 地区API模式
                    return await handleRegionAPI(request, url);
                } else if (url.pathname == `/${动态UUID}` || 路径 == `/${userID}` || url.pathname == `/${secret}`) {
                ...
                if (nat64Param) {
                    ...
                    if (providerIndex >= 0 && providerIndex < NAT64_PROVIDERS.length) {
                        const provider = NAT64_PROVIDERS[providerIndex];
                        DNS64Server = provider.dns64;
                        console.log(`使用 NAT64 服务商: ${provider.name} (${provider.dns64})`);
                    }
                }
...
async function 维列斯OverWSHandler(request) {
    // 处理WebSocket路径参数 - 类似html.js中的pyip处理
    const url = new URL(request.url);
    ...
    // 检查是否使用NAT64代理
    const nat64Param = url.searchParams.get('nat64');
    const useNAT64 = nat64Param !== null; // 当有nat64参数时启用NAT64代理
    
    console.log('维列斯OverWSHandler - NAT64参数:', nat64Param, 'useNAT64:', useNAT64, 'DNS64Server:', DNS64Server);
    ...
    if(url.pathname.includes('/pyip=')) {
        ...
        if(isValidIP(tmp_ip)) {
            ...
            console.log('WebSocket pyip 设置 ProxyIP:', localProxyIP, 'ProxyPort:', localProxyPort);
        }
    }
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            if (!banHosts.includes(addressRemote)) {
                ...
                handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote, rawClientData, webSocket, 维列斯ResponseHeader, log, localProxyIP, useNAT64);
            } else {
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, 维列斯ResponseHeader, log, wsProxyIP, useNAT64 = false) {
    ...
    async function nat64() {
        try {
            if (!useSocks) {
                console.log('NAT64 代理模式 - DNS64Server:', DNS64Server);
                try {
                    ...
                    console.error('NAT64 IPv6解析失败:', resolveError);
                    log(`NAT64 IPv6解析失败: ${resolveError.message}`);
                    // 解析失败时抛出错误，让上层回退到传统代理
                    throw new Error(`NAT64解析失败: ${resolveError.message}`);
                }
            }
            ...
            console.error('NAT64 连接失败:', error);
            log(`NAT64 连接失败: ${error.message}`);
    ...
    async function retry() {
        if (enableSocks) {
            ...
            // 如果设置了 NAT64 参数，优先使用 NAT64 代理
            log(`使用 NAT64 代理重试连接 ${addressRemote}:${portRemote}`);
            try {
                return await nat64();
            } catch (error) {
                console.error('NAT64 重试失败:', error);
                log(`NAT64 重试失败: ${error.message}`);
                // 如果 NAT64 失败，尝试使用普通代理
                let useProxyIP = wsProxyIP || proxyIP;
                if (!useProxyIP || useProxyIP == '') {
                    const defaultProxyIP = DEFAULT_PROXY_IPS[Math.floor(Math.random() * DEFAULT_PROXY_IPS.length)];
                    useProxyIP = defaultProxyIP || atob('UFJPWFlJUC50cDEuMDkwMjI3Lnh5eg==');
                }
                tcpSocket = await connectAndWrite(useProxyIP.toLowerCase() || addressRemote, portRemote);
            }
        } else {
            // 否则，尝试使用预设的代理 IP（如果有）或原始地址重试连接
            // 优先使用WebSocket传入的proxyIP，再使用全局proxyIP，最后使用默认代理IP池
            let useProxyIP = wsProxyIP || proxyIP;
            if (!useProxyIP || useProxyIP == '') {
                // 使用默认代理IP池中的随机IP
                const defaultProxyIP = DEFAULT_PROXY_IPS[Math.floor(Math.random() * DEFAULT_PROXY_IPS.length)];
            ...
            if (useProxyIP.includes('.tp')) portRemote = useProxyIP.split('.tp')[1].split('.')[0] || portRemote;
            tcpSocket = await connectAndWrite(useProxyIP.toLowerCase() || addressRemote, portRemote);
        }
    ...
    let tcpSocket;
    // 如果设置了 NAT64 参数，优先使用 NAT64 代理进行首次连接
    if (useNAT64) {
        log(`首次使用 NAT64 代理连接 ${addressRemote}:${portRemote}`);
        if (!useSocks) {
            try {
                const nat64IPv6 = await resolveToIPv6(addressRemote);
                const nat64Proxyip = `[${nat64IPv6}]`;
                ...
                console.error('首次 NAT64 解析失败，回退到传统代理:', resolveError);
                log(`NAT64 解析失败，使用传统代理: ${resolveError.message}`);
                // 回退到传统代理连接
...
async function resolveToIPv6(target) {
    ...
    try {
        ...
        if (DNS64Server.endsWith('/96')) {
            // 如果DNS64Server包含前缀，使用本地转换
            nat64 = convertToNAT64IPv6(ipv4);
        } else {
            // 否则查询DNS64服务器
            try {
                nat64 = await queryNAT64(ipv4 + atob('LmlwLjA5MDIyNy54eXo='));
            } catch (dnsError) {
                console.error('DNS64查询失败，尝试使用备用方法:', dnsError);
                // 如果DNS查询失败，尝试使用默认的前缀构造
                if (DNS64Server === '2001:67c:2960::64') {
                    // level66.services 使用固定前缀
                    const parts = ipv4.split('.');
                    const hex = parts.map(part => parseInt(part, 10).toString(16).padStart(2, '0'));
                    nat64 = `2001:67c:2960:6464:${hex[0]}${hex[1]}:${hex[2]}${hex[3]}`;
                } else if (DNS64Server === '2001:67c:2b0::4') {
                    // Trex 使用固定前缀
                    const parts = ipv4.split('.');
                    const hex = parts.map(part => parseInt(part, 10).toString(16).padStart(2, '0'));
                    nat64 = `2001:67c:2b0:db32:0:1:${hex[0]}${hex[1]}:${hex[2]}${hex[3]}`;
                } else {
                    // 其他情况抛出错误让上层处理
                    throw dnsError;
                }
        ...
        if (!isIPv6(nat64)) {
            throw new Error(`解析结果不是有效的IPv6地址: ${nat64}`);
        }
        
        return nat64;
    } catch (error) {
        console.error('NAT64解析完全失败:', error);
        console.error('目标地址:', target, 'DNS64服务器:', DNS64Server);
        // 抛出错误而不是返回后备地址，让上层决定如何处理
        throw new Error(`NAT64解析失败: ${error.message}`);
    }
}
...
131.0.72.0/22`;
    ...
    <style>
        ...
        .save-count-selector label {
            margin-right: 10px;
            font-weight: bold;
            color: #333;
        }
        .save-count-selector select {
            padding: 8px;
        ...
        .get-ip-button:disabled {
            background-color: #cccccc;
            cursor: not-allowed;
        }
        .get-ip-button:not(:disabled):hover {
    ...
    <div class="test-controls">
        ...
        <div class="button-group">
            ...
            <button class="get-ip-button" id="get-ip-btn" onclick="getIPsOnly()">仅获取IP</button>
            ...
            <button class="select-button" id="select-btn" onclick="goSelect()">返回选择页</button>
            <button class="back-button" id="back-btn" onclick="goBack()">返回配置页</button>
        </div>
        <div class="save-count-selector">
            <label for="save-count-select">保存IP条数：</label>
            <select id="save-count-select">
        ...
        <div class="save-tip">
            <strong>💡 保存提示：</strong>[<strong>覆盖保存优选IP</strong>] 和 [<strong>追加保存优选IP</strong>] 功能会根据您设置的条数保存延迟最低的优选IP。如需添加更多IP或进行自定义编辑，请使用 [<strong>编辑优选列表</strong>] 功能。
        </div>
    ...
    <script>
        ...
        function disableAllButtons() {
            ...
            const portSelect = document.getElementById('port-select');
            const ipSourceSelect = document.getElementById('ip-source-select');
            const saveCountSelect = document.getElementById('save-count-select');
        ...
        function enableButtons() {
            ...
            saveCountSelect.disabled = false;
            updateButtonStates();
        }
        ...
        async function saveIPs() {
            ...
            try {
                // 获取用户选择的保存数量
                const saveCountSelect = document.getElementById('save-count-select');
                const selectedCount = parseInt(saveCountSelect.value);
                const saveCount = selectedCount === 0 ? displayedResults.length : Math.min(displayedResults.length, selectedCount);
                ...
                if (data.success) {
                    const countText = selectedCount === 0 ? '所有' + saveCount + '个' : '前' + saveCount + '个';
                    showMessage(data.message + '（已保存' + countText + '优选IP）', 'success');
                } else {
        ...
        async function appendIPs() {
            ...
            try {
                ...
                if (data.success) {
                    const countText = selectedCount === 0 ? '所有' + saveCount + '个' : '前' + saveCount + '个';
                    showMessage(data.message + '（已追加' + countText + '优选IP）', 'success');
                } else {
        ...
        function goSelect() {
            const currentUrl = window.location.href;
            const parentUrl = currentUrl.substring(0, currentUrl.lastIndexOf('/'));
            window.location.href = parentUrl + '/select';
        }
        ...
        async function getIPsOnly() {
            ...
            ipList.innerHTML = '<div class="ip-item">正在获取IP列表，请稍候...</div>';
            ipDisplayInfo.textContent = '';
            showMoreSection.style.display = 'none';
            updateButtonStates();
            ...
            progressText.textContent = \`正在获取 \${ipSourceName} IP列表...\`;
            
            // 加载IP列表
            originalIPs = await loadIPs(selectedIPSource, selectedPort);

            if (originalIPs.length === 0) {
                ipList.innerHTML = '<div class="ip-item">获取IP列表失败，请重试</div>';
                ipCount.textContent = '0 个';
                getIPBtn.disabled = false;
                testBtn.disabled = false;
                getIPBtn.textContent = '仅获取IP';
                portSelect.disabled = false;
                ipSourceSelect.disabled = false;
                progressText.textContent = '获取失败';
                return;
            }
            ...
            getIPBtn.disabled = false;
            testBtn.disabled = false;
            getIPBtn.textContent = '重新获取IP';
            portSelect.disabled = false;
            ipSourceSelect.disabled = false;
            progressText.textContent = \`完成 - 已获取 \${originalIPs.length} 个IP (端口: \${selectedPort}, IP库: \${ipSourceName})\`;
            ...
            if (originalIPs.length > 0) {
                ...
                // 创建用于保存的结果格式（无延迟信息）
                displayedResults = originalIPs.map(ip => {
                    return {
        ...
        function displayResults() {
            ...
            displayedResults = testResults; // 保存所有结果，让用户选择保存数量
            ...
            const resultsHTML = testResults.slice(0, maxDisplayCount).map(result => {
...
async function NAT64Config(request, env) {
    const url = new URL(request.url);
    ...
    if (request.method === 'POST') {
        ...
        try {
            ...
            const selectedCountry = formData.get('country') || ''; // 获取国家选择

            let providerIndex = parseInt(selectedProvider);
            let config = {};

            if (providerIndex >= 0 && providerIndex < NAT64_PROVIDERS.length) {
                config = NAT64_PROVIDERS[providerIndex];
            } else {
                return new Response(JSON.stringify({
                    success: false,
                    error: '无效的服务商选择'
            ...
            const baseUrl = currentPath.replace('/nat64', '');
            const userPart = baseUrl.replace('/', ''); // 去掉前导的 /
            let subscriptionUrl;
            
            if (selectedCountry) {
                // 使用国家前缀的格式: https://domain/国家/xcq0607?PATH=/?ed=2560&nat64=XXX
                subscriptionUrl = `https://${hostName}/${selectedCountry}/${userPart}?PATH=%2F%3Fed%3D2560%26nat64%3D${providerIndex}`;
            } else {
                // 传统格式:  https://domain/IP/xcq0607?PATH=/?ed=2560&nat64=XXX 
                subscriptionUrl = `https://${hostName}/IP/${userPart}?PATH=%2F%3Fed%3D2560%26nat64%3D${providerIndex}`;
            }
...
<body>
    <div class="container">
        ...
        <p style="text-align: center; color: #666; margin-bottom: 30px;">
            选择不同的 NAT64 服务商来优化网络连接性能，无需重新部署！
        </p>

        <form id="nat64Form">
            <!-- 新增国家选择 -->
            <div style="margin-bottom: 30px; text-align: center;">
                <label for="country-select" style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">选择国家（可选）:</label>
                <select id="country-select" style="padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-size: 16px; min-width: 200px;">
                    <option value="">-- 不使用国家前缀 --</option>
            ...
            <div class="provider-grid">
                ${NAT64_PROVIDERS.map((provider, index) => `
                    <div class="provider-card" data-provider="${index}">
                        <div class="provider-name">${provider.name}</div>
                        <div class="provider-country">${provider.country}</div>
                        <div class="provider-details">
                            DNS64: ${provider.dns64}<br>
                            ${provider.prefix ? `Prefix: ${provider.prefix}` : ''}
                            ${provider.dot ? `<br>DoT: ${provider.dot}` : ''}
                        </div>
                    </div>
                `).join('')}
            </div>
            ...
            <div style="text-align: center; margin-top: 30px;">
                ...
                <button type="button" class="btn" onclick="location.reload()">重置选择</button>
            </div>
        </form>

        <div id="result" class="result">
            <h3>🎉 订阅链接生成成功！</h3>
            <p><strong>选择的服务商：</strong><span id="selectedProviderName"></span></p>
            <p><strong>服务商信息：</strong><span id="providerInfo"></span></p>
            <p>请使用以下订阅链接，无需重新部署即可使用所选的 NAT64 服务商：</p>
            <div class="env-vars" id="subscriptionUrl"></div>
            <button type="button" class="btn copy-btn" onclick="copySubscriptionUrl()">复制订阅链接</button>
            <button type="button" class="btn" onclick="testSubscription()" style="background: #17a2b8;">测试链接</button>
        </div>
    </div>
    ...
    <script>
        ...
        // 处理表单提交
        document.getElementById('nat64Form').addEventListener('submit', async function(e) {
            e.preventDefault();

            if (!selectedProvider) {
                alert('请选择一个 NAT64 服务商');
                return;
            }
        ...
        function showResult(result) {
            ...
            const subscriptionUrlDiv = document.getElementById('subscriptionUrl');
            const selectedProviderNameSpan = document.getElementById('selectedProviderName');
            const providerInfoSpan = document.getElementById('providerInfo');

            // 显示选择的服务商信息
            selectedProviderNameSpan.textContent = result.config.name;
            
            let infoText = result.config.country + ' - ' + result.config.dns64;
            if (result.selectedCountry) {
                infoText += ' | 国家前缀: ' + result.selectedCountry;
            }
            providerInfoSpan.textContent = infoText;

            // 显示订阅链接
            subscriptionUrlDiv.textContent = result.subscriptionUrl;
        ...
        function copySubscriptionUrl() {
            const subscriptionUrl = document.getElementById('subscriptionUrl').textContent;
...
async function handleMultiApiIntegration(request, url) {
    ...
    // 处理自定义的临时USER_ID、HOSTNAME和PATH参数
    ...
    try {
        ...
        // 构建配置URL，使用tempUserId、tempHostname和tempPath
        ...
        return new Response(`获取多API整合数据失败: ${error.message}`, { status: 500 });
    }
}
...
<head>
    ...
    <style>
        ...
        .custom-vars { display: none; margin-top: 10px; padding: 10px; border: 1px dashed #ccc; border-radius: 4px; }
        .quick-access { background-color: #e3f2fd; padding: 15px; border-radius: 5px; margin-bottom: 20px; }
        .quick-btn { background-color: #2196F3; margin: 5px; }
        .quick-btn:hover { background-color: #0b7dda; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="stats-info">
            <p>当前共有 <strong>${statsData.total || 0}</strong> 个节点地址</p>
            <p>平均延迟: <strong>${statsData.averageLatency ? statsData.averageLatency.toFixed(2) + 'ms' : '未知'}</strong></p>
            <p>最后更新时间: <strong>${statsData.lastUpdate || '未知'}</strong></p>
        </div>

        <div class="quick-access">
            <h3>🚀 快速访问入口</h3>
            <button class="quick-btn" onclick="goToSubscription()">主订阅地址</button>
            <button class="quick-btn" onclick="goToNAT64Config()">NAT64配置</button>
            <button class="quick-btn" onclick="goToEdit()">编辑配置</button>
            <button class="quick-btn" onclick="goToBestIP()">优选IP</button>
        </div>
        ...
        <div class="tabs">
            ...
            <div class="tab" data-tab="nat64-api">NAT64配置</div>
        </div>

        <div class="tab-content active" id="multi-api">
            <div class="section">
                <h3>📡 多API整合模式</h3>
                <p style="color: #666;">整合多个数据源的优选节点，提供最全面的节点覆盖。</p>
                ...
                <div class="option-group">
                    <label class="checkbox-label">
                        <input type="checkbox" id="multi-custom-vars" onchange="toggleCustomVars('multi')"> 自定义临时变量
                    </label>
                    <div id="multi-custom-vars-inputs" class="custom-vars">
                        <div class="option-group">
                            <label for="multi-user-id">自定义USER_ID:</label>
                            <input type="text" id="multi-user-id" value="${clientId}" placeholder="输入自定义USER_ID">
                        </div>
                        <div class="option-group">
                            <label for="multi-hostname">自定义HOSTNAME:</label>
                            <input type="text" id="multi-hostname" value="${serverHost}" placeholder="输入自定义域名">
                        </div>
                        <div class="option-group">
                            <label for="multi-path">自定义PATH:</label>
                            <input type="text" id="multi-path" value="${path}" placeholder="输入自定义路径">
                        </div>
                    </div>
                </div>
        ...
        <div class="tab-content" id="country-api">
            <div class="section">
                <h3>🌍 国家API模式</h3>
                <p style="color: #666;">根据特定国家获取优化的节点列表。</p>
                <div class="option-group">
                ...
                <div class="option-group">
                    <label class="checkbox-label">
                        <input type="checkbox" id="country-custom-vars" onchange="toggleCustomVars('country')"> 自定义临时变量
                    </label>
                    <div id="country-custom-vars-inputs" class="custom-vars">
                        <div class="option-group">
                            <label for="country-user-id">自定义USER_ID:</label>
                            <input type="text" id="country-user-id" value="${clientId}" placeholder="输入自定义USER_ID">
                        </div>
                        <div class="option-group">
                            <label for="country-hostname">自定义HOSTNAME:</label>
                            <input type="text" id="country-hostname" value="${serverHost}" placeholder="输入自定义域名">
                        </div>
                        <div class="option-group">
                            <label for="country-path">自定义PATH:</label>
                            <input type="text" id="country-path" value="${path}" placeholder="输入自定义路径">
                        </div>
                    </div>
                </div>
        ...
        <div class="tab-content" id="region-api">
            <div class="section">
                <h3>🗺️ 地区API模式</h3>
                <p style="color: #666;">根据地理区域获取优化的节点列表。</p>
                <div class="option-group">
                ...
                <div class="option-group">
                    <label class="checkbox-label">
                        <input type="checkbox" id="region-custom-vars" onchange="toggleCustomVars('region')"> 自定义临时变量
                    </label>
                    <div id="region-custom-vars-inputs" class="custom-vars">
                        <div class="option-group">
                            <label for="region-user-id">自定义USER_ID:</label>
                            <input type="text" id="region-user-id" value="${clientId}" placeholder="输入自定义USER_ID">
                        </div>
                        <div class="option-group">
                            <label for="region-hostname">自定义HOSTNAME:</label>
                            <input type="text" id="region-hostname" value="${serverHost}" placeholder="输入自定义域名">
                        </div>
                        <div class="option-group">
                            <label for="region-path">自定义PATH:</label>
                            <input type="text" id="region-path" value="${path}" placeholder="输入自定义路径">
                        </div>
                    </div>
                </div>
        ...
        <div class="tab-content" id="nat64-api">
            <div class="section">
                <h3>🔧 NAT64服务商配置</h3>
                <p style="color: #666;">快速访问 NAT64 配置页面，选择不同的 NAT64 服务商来优化网络连接性能。</p>
                <div class="option-group">
                    <h4>可用的NAT64服务商:</h4>
                    <ul style="color: #666; font-size: 14px;">
                        <li>Kasper Dupont (Germany, UK, Finland)</li>
                        <li>level66.services (Germany)</li>
                        <li>Trex (Finland)</li>
                        <li>ZTVI (美国 Fremont, Chicago)</li>
                        <li>Default (cmliussss)</li>
                    </ul>
                </div>
                <button onclick="goToNAT64Config()">访问 NAT64 配置页面</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleCustomVars(prefix) {
            const checkbox = document.getElementById(prefix + '-custom-vars');
            const inputsDiv = document.getElementById(prefix + '-custom-vars-inputs');
        ...
        function goToSubscription() {
            ...
            window.open(window.location.origin + '/' + accessId, '_blank');
        }
        function goToNAT64Config() {
            const accessId = ACCESS_KEY || USER_ID;
            window.open(window.location.origin + '/' + accessId + '/nat64', '_blank');
        }
        function goToEdit() {
            const accessId = ACCESS_KEY || USER_ID;
            window.open(window.location.origin + '/' + accessId + '/edit', '_blank');
        }
        function goToBestIP() {
            const accessId = ACCESS_KEY || USER_ID;
            window.open(window.location.origin + '/' + accessId + '/bestip', '_blank');
        }

        function generateMultiApiUrl() {
            const useBase64 = document.getElementById('multi-base64').checked;
            const useCustomVars = document.getElementById('multi-custom-vars').checked;
            const accessId = ACCESS_KEY || USER_ID;
            
            let url = window.location.origin + '/IP/' + accessId;
            let params = [];
            
            if (useBase64) params.push('base64');
            
            if (useCustomVars) {
                const customUserId = document.getElementById('multi-user-id').value.trim();
                const customHostname = document.getElementById('multi-hostname').value.trim();
                const customPath = document.getElementById('multi-path').value.trim();
                
                if (customUserId && customUserId !== USER_ID) {
                    params.push('USER_ID=' + encodeURIComponent(customUserId));
                }
                
                if (customHostname && customHostname !== SERVER_HOST) {
                    params.push('HOSTNAME=' + encodeURIComponent(customHostname));
                }
                
                if (customPath) {
                    params.push('PATH=' + encodeURIComponent(customPath));
                }
            }
        ...
        function generateCountryApiUrl() {
            ...
            const useCustomVars = document.getElementById('country-custom-vars').checked;
            const accessId = ACCESS_KEY || USER_ID;
            ...
            selectedCountries.forEach(country => {
                ...
                if (useCustomVars) {
                    const customUserId = document.getElementById('country-user-id').value.trim();
                    const customHostname = document.getElementById('country-hostname').value.trim();
                    const customPath = document.getElementById('country-path').value.trim();
        ...
        function generateRegionApiUrl() {
            ...
            const useCustomVars = document.getElementById('region-custom-vars').checked;
            const accessId = ACCESS_KEY || USER_ID;
            ...
            if (useCustomVars) {
                const customUserId = document.getElementById('region-user-id').value.trim();
                const customHostname = document.getElementById('region-hostname').value.trim();
                const customPath = document.getElementById('region-path').value.trim();
